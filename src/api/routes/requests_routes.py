...
async def get_request(request_id: str):
    ...
    return RequestResponse(request_id=request_id, status="pending", result=None)


@router.get("/request", response_model=RequestList)
async def list_requests():
    # Example response with one request
    example_request = RequestResponse(request_id="abc", status="pending", result=None)

    requests = [example_request]  # Lista z jednym przykładowym requestem
    return RequestList(requests=requests, total=len(requests))
